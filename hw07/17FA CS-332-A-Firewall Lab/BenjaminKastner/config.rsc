# dec/05/2017 16:20:40 by RouterOS 6.40.5
# software id = 
#
#
#
...
/ip address
add address=192.168.112.1/24 interface=ether2 network=192.168.112.0
/ip dhcp-client
...
add action=accept chain=forward dst-address=66.35.59.249 dst-port=80 \
    in-interface=ether2 protocol=tcp
add action=accept chain=forward dst-address=45.60.33.34 in-interface=ether2 \
    protocol=tcp
add action=accept chain=forward dst-address=45.60.31.34 in-interface=ether2 \
    protocol=tcp
add action=accept chain=input dst-port=80 in-interface=ether2 protocol=tcp \
    src-address=192.168.112.10
...
add action=dst-nat chain=dstnat dst-port=3389 in-interface=ether1 protocol=\
    tcp to-addresses=192.168.112.10 to-ports=3389
/system ntp client
set enabled=yes
